...
1. 优先用 `web_search` 搜索独立证据。执行信息核查时，默认传入：
   - `max_results=8`
   - 中文事件优先使用 `region="cn-zh"`，国际或英文事件使用 `region="wt-wt"`
   - `web_search` 会自动优先搜索官方、权威媒体、研究机构等来源，并按来源权威性重排结果
...
3. 如果 `web_search` 结果不足，且当前可用工具包含 `mcp__firecrawl_search`，可再使用 `mcp__firecrawl_search` 补充搜索；若触发权限确认，按权限层结果处理。
...
- 每条 `verdict` 必须尽量包含 `evidence_sources`，把 `web_search` 返回的 `url`、`domain`、`source_type`、`authority_score`、`matched_authority_rule` 等字段保留下来。
...
每条主张必须优先调用 `web_search` 搜索。该工具默认会优先搜索权威来源并按来源权威性重排：
...
如果默认权威优先搜索没有返回可用证据，再换用更宽泛的关键词搜索。宽泛搜索只能作为补充线索，不能覆盖官方来源或一手证据。
